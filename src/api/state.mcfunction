...
/load
  ...
  /final
    ...
    %FILE%/load/pos with storage game:math temp.pos
  /pos
  ...
  #END
    ...
    %FILE%/load/xp with storage game:math temp
  /xp
    xp set @s 10000 levels
    $xp set @s $(XpP) points
    $xp set @s $(XpLevel) levels
  #END


    execute if data storage game:math temp.State.attributes[0] run %FILE%/load/attributes

  /attributes

    %FILE%/load/attributes/macro with storage game:math temp.State.attributes[0]

    data remove storage game:math temp.State.attributes[0]
    execute if data storage game:math temp.State.attributes[0] run %FILE%/load/attributes
  ...
  #tellraw @a {nbt:"temp.State.Health",storage:"game:math"}
...
/force_inventory
  ...
  $execute as $(target) run %FILE%/load {dim:"$(dim)"}
  %FILE%/load {dim:"backupSpy"}
#END


/clear_player_data
  data modify storage game:player data set value []
  tag @a remove Game.PlayerInited
#END


/backup
  execute if predicate game:dimension/main run %FILE%/save {dim:"main"}
  execute if predicate game:dimension/game_scape run %FILE%/save {dim:"game_scape"}
  execute if predicate game:dimension/dev_world run %FILE%/save {dim:"dev_world"}
